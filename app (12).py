...
if page == "🎵 Streaming Performance":
    ...
    st.plotly_chart(
        px.bar(v1, x="streams", y="artist_name", orientation="h", color_discrete_sequence=["#1DB954"]),
        use_container_width=True
    )
    st.markdown("<div class='insight-box'>Streaming volume is highly concentrated among a small set of top artists.</div>", unsafe_allow_html=True)
    ...
    st.plotly_chart(
        px.pie(v2, names="genre", values="streams", hole=0.45,
    ...
    st.plotly_chart(
        px.line(v3, x="month", y="streams", markers=True, color_discrete_sequence=["#1DB954"]),
        use_container_width=True
    )
    st.markdown("<div class='insight-box'>Seasonal peaks guide release timing and campaign planning.</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='insight-box'>Premium users generate a disproportionate share of total streams.</div>", unsafe_allow_html=True)
...
# =====================================================
    ...
    st.plotly_chart(
        px.bar(cdf, x="campaign_name", y="net_user_gain", color_discrete_sequence=["#1DB954"]),
        use_container_width=True
    )
    st.markdown("<div class='insight-box'>Identifies winning campaigns and underperformers.</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='insight-box'>Consistent uplift matters more than short-lived spikes.</div>", unsafe_allow_html=True)

    # -------- VISUAL 3 --------
    st.subheader("User Growth by Target Genre")
    v3 = cdf.groupby("target_genre").net_user_gain.sum().reset_index()
    st.plotly_chart(
        px.bar(v3, x="target_genre", y="net_user_gain", color_discrete_sequence=["#1DB954"]),
        use_container_width=True
    )
    st.markdown("<div class='insight-box'>Genre-led campaigns reveal where growth converts best.</div>", unsafe_allow_html=True)

    # -------- VISUAL 4 (PERMANENTLY FIXED) --------
    ...
    st.plotly_chart(
        px.scatter(
            ...
            size="net_user_gain",
            color="user_growth_pct",  # numeric color => SAFE
            color_continuous_scale="Greens"
        ),
        use_container_width=True
    )
    st.markdown("<div class='insight-box'>High acquisition without premium uplift highlights monetization gaps.</div>", unsafe_allow_html=True)
